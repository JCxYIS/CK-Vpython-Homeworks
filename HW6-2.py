...
size = 2              #球半徑2 m
height_1 = 30.0       #球1初始高度
height_2 = height_1-2*size       #球2初始高度
height_3 = height_2-2*size
...
ball_3 = sphere(radius = size, color=color.blue ) #畫球
...
ball_3.pos = vector( 0, size+height_3, 0)        #球初始位置       
...
while True:             
    ...
    if ball_3.y <= size and ball_3.v.y < 0:     
    ...
    if abs(ball_2.y-ball_3.y) <= 2*size :
        v1y = (m2-m3)*ball_2.v.y/(m2+m3) + 2*m3*ball_3.v.y/(m3+m2)
        v2y = 2*m2*ball_2.v.y/(m3+m2) + (m3-m2)*ball_3.v.y/(m3+m2)
